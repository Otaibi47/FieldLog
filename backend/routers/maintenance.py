...
async def list_logs(equipment_id: str | None = None, db: AsyncSession = Depends(get_db), _=Depends(verify_token)):
...
async def create_log(body: MaintenanceLogCreate, db: AsyncSession = Depends(get_db), _=Depends(verify_token)):
...
async def get_log(log_id: str, db: AsyncSession = Depends(get_db), _=Depends(verify_token)):
